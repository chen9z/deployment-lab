...
# `bash eval/verify-stress.sh` (5-10 min, KV-cache + prefill boundary tests).
#
# Checks (in order):
#   1. Server reachable
...
# prefill OOM detection): see eval/verify-stress.sh
#
# Usage:
#   CONTAINER=<your-container> bash eval/verify-full.sh
#
# Env (optional):
...
#
# Optional flag:
#   --bench      After all correctness checks pass, run eval/bench.sh
...
check_server() {
  ...
  if curl -sf -m 5 "${URL}/v1/models" >/dev/null 2>&1; then
    ...
    fail "no response from ${URL}/v1/models" \
         "Start the stack: docker compose -f qwen3.5-27b/docker-compose.yml up -d ; docker logs -f ${CONTAINER}"
  fi
}
...
if [[ "$RUN_BENCH" == "1" && "$FAILED" == "0" ]]; then
  echo ""
  echo "=========================================="
  echo "  --bench: running eval/bench.sh"
  echo "=========================================="
