...
#
# Env vars:
#   URL                Endpoint. Default: http://127.0.0.1:8001
#   MODEL              Served model name. Default: Qwen3.5-27B
...
# Usage:
#   bash eval/bench.sh
#   ONLY=code bash eval/bench.sh
#   PP=1 bash eval/bench.sh
#   RUNS=10 bash eval/bench.sh
...
if ! curl -sf "${URL}/v1/models" >/dev/null; then
  ...
  echo "  Start with: docker compose -f qwen3.5-27b/docker-compose.yml up -d" >&2
  exit 1
fi
