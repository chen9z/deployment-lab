...
class RoutedExpertsReader:
    """Compatibility shim for vLLM builds that import this optional helper.

    The Gemma 4 31B serving profile does not enable return_routed_experts, so
    the scheduler only needs this symbol to exist at import time.
    """

    @staticmethod
    def create():
        return RoutedExpertsReader()

    def attach_buffer(self, *args, **kwargs):
        raise NotImplementedError("return_routed_experts is not enabled")
