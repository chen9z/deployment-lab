...
class JinaV5EmbeddingModel(EmbeddingModelWrapper):
    """Wrapper for jina-embeddings-v5 retrieval checkpoints using last-token pooling."""
    ...
    async def load(self):
        logger.info("Loading Jina v5 embedding model: %s", self.model_name)
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(
                self.model_name,
                trust_remote_code=True,
            ...
            if torch.cuda.is_available():
                ...
                logger.info("CUDA not available, using CPU")
            self.model.eval()
            logger.info("Successfully loaded Jina v5 embedding model")
        except Exception as exc:
            logger.error("Failed to load Jina v5 embedding model: %s", exc)
            raise
    ...
    async def encode(self, texts: List[str]) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            if mask is None:
                raise RuntimeError("Tokenizer output does not contain attention_mask")
