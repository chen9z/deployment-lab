...
MODEL_DIR="${MODEL_DIR:-$ROOT_DIR/models/cyankiwi/gemma-4-26B-A4B-it-AWQ-4bit}"
BASE_URL="${BASE_URL:-http://127.0.0.1:8006}"
MODEL_NAME="${MODEL_NAME:-gemma-4-26B-A4B}"
OUT_DIR="${OUT_DIR:-$ROOT_DIR/gemma-4-26b/single_3090_awq_bench_$(date +%Y%m%d_%H%M%S)}"
NUM_PROMPTS_SINGLE="${NUM_PROMPTS_SINGLE:-12}"
NUM_PROMPTS_MULTI="${NUM_PROMPTS_MULTI:-24}"
INPUT_LEN="${INPUT_LEN:-1024}"
OUTPUT_LEN="${OUTPUT_LEN:-256}"
REQUEST_RATE="${REQUEST_RATE:-inf}"
MULTI_CONCURRENCY="${MULTI_CONCURRENCY:-2}"
...
if ! curl -fsS --max-time 5 "$BASE_URL/v1/models" > "$OUT_DIR/models.json"; then
  if [[ -n "${HOST_IP:-}" ]]; then
    ALT_BASE_URL="http://${HOST_IP}:8021"
...
run_case "throughput_c${MULTI_CONCURRENCY}_in${INPUT_LEN}_out${OUTPUT_LEN}" "$MULTI_CONCURRENCY" "$NUM_PROMPTS_MULTI"
