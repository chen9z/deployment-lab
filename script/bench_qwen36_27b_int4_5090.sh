...
CONTAINER_NAME="${CONTAINER_NAME:-vllm-qwen36-27b}"
BASE_URL="${BASE_URL:-http://127.0.0.1:8001}"
MODELS_URL="${MODELS_URL:-${BASE_URL%/}/v1/models}"
CONTAINER_BASE_URL="${CONTAINER_BASE_URL:-http://127.0.0.1:8001}"
TOKENIZER_PATH_IN_CONTAINER="${TOKENIZER_PATH_IN_CONTAINER:-/models/qwen36-27b}"
PROMPT_LEN="${PROMPT_LEN:-64}"
OUTPUT_LEN="${OUTPUT_LEN:-256}"
REQUEST_RATE="${REQUEST_RATE:-inf}"
NUM_WARMUPS="${NUM_WARMUPS:-0}"
PROMPTS_PER_CONCURRENCY="${PROMPTS_PER_CONCURRENCY:-8}"
OUT_DIR="${OUT_DIR:-$ROOT_DIR/qwen3.6-27b/qwen36_27b_vllm_bench_$(date +%Y%m%d_%H%M%S)}"
CONTAINER_OUT_DIR="${CONTAINER_OUT_DIR:-/tmp/$(basename "$OUT_DIR")}"

# Default to the short-context sweep used for the current 5090 checks.
DEPTH_VALUES=(${DEPTH_VALUES:-0 4096 16384})
...
for json_path in sorted(glob(os.path.join(result_dir, "c*_depth*_in*_out*.json"))):
    ...
    prompt_len = int(match.group("prompt")) if match.group("prompt") else default_prompt_len
    output_len = int(match.group("output")) if match.group("output") else default_output_len
...
with open(summary_path, "w", encoding="utf-8") as f:
    f.write(
        ...
        "duration\trequest_throughput\toutput_throughput\ttotal_token_throughput\tmax_output_tokens_per_s\t"
    ...
    for row in rows:
        f.write(
            ...
            f"{row['max_output_tokens_per_s']}\t{row['mean_ttft_ms']}\t{row['p99_ttft_ms']}\t"
            f"{row['mean_tpot_ms']}\t{row['p99_tpot_ms']}\t{row['mean_itl_ms']}\t{row['p99_itl_ms']}\t"
